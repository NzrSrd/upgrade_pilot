...
def route_after_plan(state: RAGState) -> Literal["retrieve", "build_context"]:
    """Skip the whole loop when there is nothing to look up.

    Reads the flag `plan_retrieval` set rather than re-deriving the condition,
    so the decision is taken once, in the node that recorded its reason in the
    trace. A predicate that recomputed "is the inventory empty?" here would be
    a second implementation of the same rule with no trace event behind it,
    and the two could disagree after any change to what counts as a usage
    site.
    """
    return "retrieve" if state["retrieval_necessary"] else "build_context"
...
def route_after_evaluate(state: RAGState) -> Literal["plan_retrieval", "build_context"]:
    ...
    `>=` rather than `==` on the budget: `max_iterations` arrives from
    configuration, and a value of zero must stop the loop rather than run it
    forever looking for equality it will never reach.
    """
