...
- **Related:** `docs/superpowers/specs/2026-08-24-upgradepilot-agent-core-design.md`, `docs/adr/ADR-002-deployment.md`
...
| LangGraph checkpointer | thread/run state; enables interrupt/resume **and** serves status polling | `AsyncSqliteSaver`. ADR-002 D2 (**Proposed**, not yet implemented — Phase 13) adds an `AsyncPostgresSaver` backend selected by `UP_CHECKPOINT_URL`, because a hosted deployment on an in-memory filesystem would hold paused runs in RAM. SQLite remains the local and test path. |
| PostgreSQL | accounts, analysis history | absent — arrives in Sub-project 3. Narrowed by ADR-002 D2 (**Proposed**): the *checkpointer* is planned to reach Postgres early, for deployment reasons. Accounts and analysis history are unaffected and stay in Sub-project 3. |
...
`RepoRef` resolves to a `Workspace` — the only thing the analyzer ever sees. Spec 1 provides a shallow-clone resolver (public URLs, no credentials) and a local-path resolver. Sub-project 2 adds authenticated clones without touching the analyzer. **Where the credential comes from is now settled early:** ADR-002 D4 (**Proposed**) puts Clerk in the deployment with GitHub as the social connection, and Clerk holds the signed-in user's GitHub token. So Sub-project 2's remaining work is the clone path itself, not an OAuth implementation — and this row's claim that the analyzer is untouched is the reason that stays a contained change.
...
- Spec 1 runs single-worker. The in-memory run registry is wrong under multiple uvicorn workers. Documented as a constraint; Sub-project 3 moves the registry into Postgres and lifts it. **Which component pins this matters, and ADR-002 D2 makes it easy to misread:** it is the *registry*, not the checkpointer. Moving thread state to Postgres makes a paused run durable and shared; it does **not** make a second process able to see the first process's `RUNNING` set, which is what a status poll needs. A deployment must therefore assert a single instance even once the checkpointer is shared — ADR-002 D1 is where that is applied.
...
| Phase 13 pinned versions (ADR-002) | Rule 13, for the dependencies ADR-002 adds. **Resolved and installed this session:** `langgraph-checkpoint-postgres 3.1.2 · psycopg 3.3.5 · psycopg-binary 3.3.5 · psycopg-pool 3.3.1`, alongside `aiosqlite 0.22.1` which was already present as a transitive and is now declared. No conflict: the Postgres saver requires `langgraph-checkpoint>=4.1.0,<5.0.0` and `orjson>=3.11.5`, satisfied by the installed `4.2.0` and `3.12.0` without moving either. `psycopg[binary]` was chosen over bare `psycopg` because the latter needs a system `libpq`; the binary distribution bundles it and ships a genuine `cp314` wheel. The suite was re-run with all four present and is unchanged at **1173 passed / 8 skipped**, ruff and mypy clean. **Not yet resolved:** `clerk-backend-api` (ADR-002 D4) is verified *available* but is not installed, so its version is not recorded here — `7.0.0` requires `cryptography>=45,<51`, `pyjwt>=2.9,<3`, `httpx>=0.28.1` and `pydantic>=2.11.2`, the last of which the installed `2.13.4` satisfies. Note that adopting it moves `httpx` from a dev-only dependency to a runtime one. *Source: `pip install` and `pip list --format=freeze` run this session; `probes/probe_wheels.py`.* |
| linux/amd64 wheel availability | **Every platform-specific package in this project resolves on `linux/amd64` for CPython 3.14 from a `manylinux` wheel — none needs a source build, and none is `musllinux`-only.** Measured across the **38** distributions in the venv whose `WHEEL` tags are not pure-Python at the time of measurement — the count moves with the dependency set, so re-run the probe rather than trusting this number — including the ones most likely to lag a new interpreter: `onnxruntime 1.29.0`, `numpy 2.5.3`, `grpcio 1.83.1`, `tokenizers 0.23.2`, `pydantic-core 2.48.0`. This is what makes ADR-002's Debian `python:3.14-slim` base and the CI matrix viable, and it is why neither needed a fallback plan. **Two corrections worth keeping, because the probe was wrong twice before it was right, in both cases by over-claiming absence.** (1) It first reported `ruff` and `sqlite-vec` as having no linux wheel: both ship a native binary that exposes no Python C-API, so their wheels carry a `py3` tag beside a *platform* tag (`py3-none-manylinux_2_17_x86_64`) — the exact shape the Pinned-versions row above already records for macOS, and a classifier keying only on `abi3`/`cp314` calls them missing. (2) Its linux filter matched the substring `linux`, which also matches `musllinux`, and would have passed a wheel that cannot load on a glibc base. Tightened to require `manylinux`, and the result held. *Source: `probes/probe_wheels.py`, run this session against PyPI.* |
| Postgres checkpointer across a real process restart | **A paused run survives a process boundary, which is the single claim ADR-002 D2 rests on.** `AsyncPostgresSaver` was driven to `interrupt()` in one interpreter (pid 82488) which then exited, taking its connection pool and compiled graph with it; a second interpreter (pid 82836) that had never seen that graph object opened its own saver over the same database, found the checkpoint (`state.next == ("review",)`, one interrupt on the pending task), resumed with a decision, and reached `trace == ["first", "review", "last:b"]` with `next == ()`. This is deliberately **stronger than** the SQLite row above, which says of itself: "This is disk durability across a closed connection; it is not a process restart, since the test runs in one process." The graph is copied from `tests/graph/test_langgraph_contract.py` rather than invented, so a difference in outcome would be a difference in the checkpointer. One property re-confirmed in passing: the `review` node body **re-executed** on resume, so work placed before an `interrupt()` is performed once per resume under Postgres exactly as under SQLite — the rule `graph/nodes/judgment.py` already obeys by enumerating strategies in `assess_risk` instead. The mitigation therefore carries over unchanged. *Source: `probes/probe_postgres_checkpointer.py pause` then `resume`, run this session against PostgreSQL 17.11.* |
| Postgres checkpointer across an **idle disconnect** | **A held connection dies before the state does, and the two are separate guarantees.** The row above proves a paused run survives a *process* restart. It does not cover the opposite arrangement — the process living and the connection dying — which is the ordinary overnight state of the deployment ADR-002 describes, because Neon suspends its compute after five minutes idle while Cloud Run keeps an idle instance alive. Measured: `AsyncPostgresSaver.from_conn_string` holds **one** `AsyncConnection` for the application lifespan, and after `pg_terminate_backend` the next write raised `psycopg.OperationalError: consuming input failed: server closed the connection unexpectedly` — a 500 on the resume of a run reported as safely paused. The identical defect was present in `open_ownership`, surfacing as `psycopg.errors.AdminShutdown` raised from `require_owner` (a failed authorisation check, so unavailable rather than unsafe). **Fixed with `AsyncConnectionPool(check=AsyncConnectionPool.check_connection)`, and the pool is not the fix on its own** — with the pool present and `check=` removed the reproduction still fails, logging `discarding closed connection`, because the pool inspects a connection when it is returned rather than when it is handed out. *Source: `tests/graph/test_checkpointer_survives_an_idle_disconnect.py` and `tests/api/test_run_ownership.py::test_ownership_still_answers_after_the_database_hangs_up`, both run this session against local Postgres 17.11; suite 1204 passed / 8 skipped.* |
| Deployed image size (ADR-002 D5) | **231,700,255 bytes (~221 MiB)** for the backend image with the corpus baked in, tag `f581b31`. Recorded because ADR-002 D5 accepts a cold start as the price of scaling to zero, and that acceptance was originally made with the size unmeasured. At this size the image pull is not the dominant term in a cold start, which leaves the `chromadb` import as the thing to attack if cold starts become a complaint. *Source: `gcloud artifacts docker images list europe-west1-docker.pkg.dev/project-1d421ae2-fa0b-4a56-a6c/upgradepilot/backend`, run this session.* |
